/**
 * MultiSelect - A lightweight multi-select component for Bootstrap 5
 * Version: 2.1.0
 * Dependencies: jQu        createOptionElement(option, index) {
            const $optionDiv = $('<div class="multi-select-option" role="option" tabindex="-1"></div>')
                .data('value', option.value)
                .data('index', index)
                .data('icon', option.icon || '') // Store icon reference
                .attr('aria-selected', option.selected.toString());
            
            if (this.isMultiple) {
                $optionDiv.append(
                    $('<input type="checkb        createTag(text, value) {
            ...
            if (icon) {
                ...
                // Check if it's an image URL or an icon class
        ...
        },ss="form-check-input" aria-hidden="true">')
            ...
            if (option.icon) {
                ...
                // Check if it's an image URL or an icon class
                if (option.icon.startsWith('http') || option.icon.startsWith('data:') || 
                    option.icon.startsWith('./') || option.icon.startsWith('/')) {
                    $iconDiv.append($('<img>').attr('src', option.icon).attr('alt', ''));
                } else {
                    $iconDiv.append($('<i>').addClass(option.icon));
                }
                
                $optionDiv.append($iconDiv);
            }
            
            $optionDiv.append($('<span class="multi-select-option-text"></span>').text(option.text));
            
            if (option.selected) $optionDiv.addClass('selected');
    ...
    class MultiSelect {
        ...
        createOptions() {
            ...
            this.$element.find('option').each((index, option) => {
                // Create option object to support icons
                const optionObj = {
                    value: option.value,
                    text: option.text,
        ...
        createOptionElement(option, index) {
            // Handle both DOM option elements and option objects
            let optionValue, optionText, optionSelected, optionIcon;
            
            if (option instanceof HTMLOptionElement) {
                // It's a DOM option element
            ...
            const $optionDiv = $('<div class="multi-select-option" role="option" tabindex="-1"></div>')
                ...
                .data('index', index)
                .data('icon', optionIcon) // Store icon reference
                .attr('aria-selected', optionSelected.toString());
            
            // Create left content container for icon and text
        ...
        // Focus navigation
        focusNextOption() {
            const options = this.$optionsContainer.find('.multi-select-option:visible');
        ...
        toggleOption($optionElement) {
            ...
            if (isSelected) {
                ...
                if (!this.isMultiple) {
                    ...
                    // Multiple select - check max selection
            ...
            if (this.isMultiple) {
                ...
                if (!this.selectedValue) {
                    ...
                    if ($option.length) {
                        ...
                        // If not found in dropdown, try select element
                        if (!icon) {
        ...
        createTag(text, value) {
            ...
            // Check if option has an icon - try multiple sources
            let icon = null;
            
            // First try to get from dropdown option element
            const $dropdownOption = this.$optionsContainer.find(`[data-value="${value}"]`);
            if ($dropdownOption.length) {
                icon = $dropdownOption.data('icon');
            }
            
            // If not found, try to get from select option element
            if (!icon) {
        ...
        populateOptions(options = []) {
            ...
            options.forEach((option, index) => {
                ...
                // Store icon as data attribute if available
                if (option.icon) {
                ...
                // Create extended option object with icon
                const optionObj = {
                    value: option.value,
                    text: option.text || option.label,
                    selected: !!option.selected,
                ...
                // Create option for dropdown with icon support
